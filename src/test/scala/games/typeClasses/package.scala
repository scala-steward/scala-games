...
package object typeClasses extends Specification {
  def encoderDecoderParity[T: Encoder: Decoder](values: NonEmptyList[(T, String)]) = {
    values.map(_._1).map(DecoderRules.reflectiveToEncoder[T](_)).reduce[MatchResult[Option[T]]](_ and _) and
      values.map(_._2).map(EncoderRules.reflectiveToDecoder[T](_)).reduce[MatchResult[String]](_ and _) and
