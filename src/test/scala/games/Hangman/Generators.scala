...
object Generators {
  ...
  val guessesGen: Gen[Ior[NonEmptySet[Char], NonEmptySet[NonEmptyList[Char]]]] =
    for {
      symbols <- Gen
        .nonEmptyListOf(Gen.alphaChar)
        .map(ls => symbolChoices(NonEmptyList.fromListUnsafe(ls)))
      words <- Gen
        .nonEmptyListOf(Gen.nonEmptyListOf(Gen.alphaChar))
        .map(ls =>
          wordChoices(
            NonEmptyList.fromListUnsafe(ls.map(NonEmptyList.fromListUnsafe))))
    } yield symbols.combine(words)
  implicit val arbitraryGuesses
    : Arbitrary[Ior[NonEmptySet[Char], NonEmptySet[NonEmptyList[Char]]]] =
    Arbitrary(guessesGen)

  val wordGen: Gen[NonEmptyList[Char]] =
    Gen.nonEmptyListOf(Gen.alphaChar).map(NonEmptyList.fromListUnsafe)
  ...
  implicit val arbitraryState: Arbitrary[State] = {
    ...
    val genGame = for {
      ...
      Game(lives,
           word,
           guesses.map(choices =>
             symbolChoices(NonEmptyList.fromListUnsafe(choices))))
  ...
  implicit val arbitraryInput: Arbitrary[Input] = {
    ...
    val genRestart =
      Gen.oneOf(const(Easy), const(Medium), const(Hard)).map(Restart)
  ...
  implicit val arbitraryDifficultyLevel: Arbitrary[DifficultyLevel] = Arbitrary(
    Gen.oneOf(const(Easy), const(Medium), const(Hard))
  )

  val arbitraryWordWithNonMatchingSymbol
    : Arbitrary[(NonEmptyList[Char], Char)] = Arbitrary(for {
    symbol <- Gen.numChar
    word <- wordGen
    if !word.toNes.contains(symbol)
  } yield (word, symbol))

  val arbitraryWordWithMatchingSymbol: Arbitrary[(NonEmptyList[Char], Char)] =
    Arbitrary(for {
      symbol <- Gen.alphaChar
      word <- wordGen
    } yield (symbol :: word, symbol))

  def unEvenPair[T: Arbitrary]: Arbitrary[(T, T)] =
    Arbitrary(Arbitrary.arbitrary[(T, T)].suchThat(tp => tp._1 != tp._2))
