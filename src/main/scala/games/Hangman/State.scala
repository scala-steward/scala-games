...
case class Game(lives: Lives, word: Word, guesses: Option[Guesses])
    extends State
...
object State {
  ...
  implicit val lifeEncoder: Encoder[Lives] = new Encoder[Lives] {
    ...
    override def encode(value: Lives): String =
      pieces
        .drop(value.lives)
        .foldLeft(emptyGameScene) {
          case (drawing, Piece((x, y), symbol)) =>
            drawing.updated(x, drawing(x).updated(y, symbol))
        }
        .map(_.mkString(""))
        .mkString("\n")
  }
  ...
  implicit val stateEncoder: Encoder[State] = {
    case Init => info
    case Win(word) =>
      s"Congratulations, you won. The word was: ${word.toList.mkString("")}"
    case Lose(word, guesses) =>
      s"You lost. The word was ${word.toList.mkString("")} and your guesses were ${guessesToStr(guesses)}"
    case Game(lives, word, guesses) =>
      List(
        List(lives.encode),
        List(s"word: ${word
          .map(chr =>
            if (guesses.flatMap(_.left).exists(_.exists(_ == chr))) chr
            else '_')
          .toList
          .mkString(" ")}"),
