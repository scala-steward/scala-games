...
import games.Hangman.WordService
...
object Main extends IOApp {
  ...
  case object ApiWordService extends WordService[IO] {
    override def getWord: IO[NonEmptyList[Char]] =
      for {
        wordApiResponse <- IO(
          sttp
            .get(uri"https://raw.githubusercontent.com/dwyl/english-words/master/words.txt")
            .send())
        ...
        word <- if (randomWord.isDefined) IO(randomWord.get)
        else
          IO.raiseError(new RuntimeException("Word API returning weird values"))
      } yield word
  }
